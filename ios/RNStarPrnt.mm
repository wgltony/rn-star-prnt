...
- (void)startObserving {
    ...
    // Set up any upstream listeners or background tasks as needed
}

// Called when the last listener is removed or on dealloc.
- (void)stopObserving {
    RNStarPrnt_hasListeners = NO;
    // Remove upstream listeners or stop background tasks as needed
}

#pragma mark - React Native Methods

RCT_REMAP_METHOD(portDiscovery,
                 portType:(NSString *)portType
                 ...
                 rejecter:(RCTPromiseRejectBlock)reject)
{
    ...
    // Discover printers using Bluetooth
    if ([portType isEqualToString:@"All"] || [portType isEqualToString:@"Bluetooth"]) {
        NSArray *btPortInfoArray = [SMPort searchPrinter:@"BT:" :&error];
        if (error) {
            NSLog(@"Error searching for Bluetooth printers: %@", error);
        }
    ...
    if ([portType isEqualToString:@"All"] || [portType isEqualToString:@"LAN"]) {
        error = nil;
        NSArray *lanPortInfoArray = [SMPort searchPrinter:@"TCP:" :&error];
        if (error) {
            NSLog(@"Error searching for LAN printers: %@", error);
        }
    ...
    // Discover printers using BluetoothLE
    if ([portType isEqualToString:@"All"] || [portType isEqualToString:@"BluetoothLE"]) {
        error = nil;
        NSArray *blePortInfoArray = [SMPort searchPrinter:@"BLE:" :&error];
        if (error) {
            NSLog(@"Error searching for BluetoothLE printers: %@", error);
        }
        for (PortInfo *p in blePortInfoArray) {
            [info addObject:[self portInfoToDictionary:p]];
        }
    }
    
    // Discover printers on USB
    if ([portType isEqualToString:@"All"] || [portType isEqualToString:@"USB"]) {
        error = nil;
        NSArray *usbPortInfoArray = [SMPort searchPrinter:@"USB:" :&error];
        if (error) {
            NSLog(@"Error searching for USB printers: %@", error);
        }
                 ...
                 rejecter:(RCTPromiseRejectBlock)reject)
{
    ...
    @try {
        // Open the port using the modern API with an error pointer
        port = [SMPort getPort:portName :portSettings :10000 :&error];
        if (port == nil || error) {
            NSLog(@"Error opening port %@: %@", portName, error);
            @throw error;
        }
        
        // Pause to avoid Bluetooth communication issues on iOS 11+
        ...
        // Retrieve the printer status using the modern API with an error pointer
        if (![port getParsedStatus:&status :2 :&error]) {
            NSLog(@"Error retrieving status for port %@: %@", portName, error);
            @throw error;
        }
        
        // Get firmware information
        NSDictionary *firmwareInformation = nil;
        @try {
            firmwareInformation = [port getFirmwareInformation:&error];
            if (firmwareInformation == nil) {
                firmwareInformation = @{};
            }
        }
        @catch (NSException *exception) {
            NSLog(@"Unable to retrieve firmware information: %@", exception);
            firmwareInformation = @{};
        }
        
        NSDictionary *statusDictionary = [self portStatusToDictionary:status :firmwareInformation];
        NSLog(@"Printer status successfully retrieved");
        resolve(statusDictionary);
    }
    @catch (NSError *exception) {
        NSLog(@"Exception while retrieving port status: %@", exception);
                 ...
                 rejecter:(RCTPromiseRejectBlock)reject)
{
    ...
    if ([_printerManager connect] == NO) {
        NSDictionary *userInfo = @{
            NSLocalizedDescriptionKey: NSLocalizedString(@"Failed to open port", nil),
            NSLocalizedFailureReasonErrorKey: NSLocalizedString(@"", nil),
            NSLocalizedRecoverySuggestionErrorKey: NSLocalizedString(@"Check that the printer is online", nil)
        };
        NSError *error = [[NSError alloc] initWithDomain:@"StarPrntErrorDomain" code:-1 userInfo:userInfo];
                 ...
                 rejecter:(RCTPromiseRejectBlock)reject)
{
    if (_printerManager != nil && _printerManager.port != nil) {
        if ([_printerManager disconnect] == NO) {
            ...
            NSError *error = [[NSError alloc] initWithDomain:@"StarPrntErrorDomain" code:-1 userInfo:userInfo];
            NSLog(@"Error disconnecting printer");
        ...
        [Communication sendCommands:[builder.commands copy]
                  ...
                  completionHandler:^(BOOL result, NSString *title, NSString *message) {
                      if (result == YES) {
                          ...
                          NSLog(@"Print command sent successfully (direct connection)");
                          resolve(resultMessage);
                      } else {
                          NSDictionary *userInfo = @{
                              NSLocalizedDescriptionKey: NSLocalizedString(title, nil),
                              NSLocalizedFailureReasonErrorKey: NSLocalizedString(message, nil),
                              NSLocalizedRecoverySuggestionErrorKey: NSLocalizedString(@"Check that the printer is online and connected", nil)
                          };
                          NSError *error = [[NSError alloc] initWithDomain:@"StarPrntErrorDomain" code:-1 userInfo:userInfo];
                          NSLog(@"Error sending print command: %@", message);
                          reject(title, message, error);
                      }
                  }];
    } else { // Use StarIOExtManager to send commands to the connected printer
        [Communication sendCommands:[builder.commands copy]
                  ...
                  completionHandler:^(BOOL result, NSString *title, NSString *message) {
                      if (result == YES) {
                          ...
                          [resultMessage setObject:title forKey:@"title"];
                          [resultMessage setObject:message forKey:@"message"];
                          NSLog(@"Print command sent successfully (via StarIOExtManager)");
                          resolve(resultMessage);
                      } else {
                          NSDictionary *userInfo = @{
                              NSLocalizedDescriptionKey: NSLocalizedString(title, nil),
                          ...
                          NSError *error = [[NSError alloc] initWithDomain:@"StarPrntErrorDomain" code:-1 userInfo:userInfo];
                          NSLog(@"Error sending print command via StarIOExtManager: %@", message);
                          reject(title, message, error);
                      }
                  }];
    }
...
- (NSMutableDictionary *)portStatusToDictionary:(StarPrinterStatus_2)status :(NSDictionary *)firmwareInformation {
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
    [dict setObject:@(status.coverOpen == SM_TRUE) forKey:@"coverOpen"];
    [dict setObject:@(status.offline == SM_TRUE) forKey:@"offline"];
    [dict setObject:@(status.overTemp == SM_TRUE) forKey:@"overTemp"];
    [dict setObject:@(status.cutterError == SM_TRUE) forKey:@"cutterError"];
    [dict setObject:@(status.receiptPaperEmpty == SM_TRUE) forKey:@"receiptPaperEmpty"];
...
- (void)appendCommands:(ISCBBuilder *)builder printCommands:(NSArray *)printCommands {
    NSStringEncoding encoding = NSASCIIStringEncoding;
    
    for (id command in printCommands) {
        if ([command valueForKey:@"appendInternational"])
            [builder appendInternational:[self getInternational:[command valueForKey:@"appendInternational"]]];
        else if ([command valueForKey:@"appendCharacterSpace"])
            [builder appendCharacterSpace:[[command valueForKey:@"appendCharacterSpace"] intValue]];
        else if ([command valueForKey:@"appendEncoding"])
            encoding = [self getEncoding:[command valueForKey:@"appendEncoding"]];
        else if ([command valueForKey:@"appendCodePage"])
            [builder appendCodePage:[self getCodePageType:[command valueForKey:@"appendCodePage"]]];
        else if ([command valueForKey:@"append"])
            [builder appendData:[[command valueForKey:@"append"] dataUsingEncoding:encoding]];
        else if ([command valueForKey:@"appendRaw"])
            [builder appendRawData:[[command valueForKey:@"appendRaw"] dataUsingEncoding:encoding]];
        else if ([command valueForKey:@"appendEmphasis"])
            [builder appendDataWithEmphasis:[[command valueForKey:@"appendEmphasis"] dataUsingEncoding:encoding]];
        else if ([command valueForKey:@"enableEmphasis"])
            [builder appendEmphasis:[[command valueForKey:@"enableEmphasis"] boolValue]];
        else if ([command valueForKey:@"appendInvert"])
            [builder appendDataWithInvert:[[command valueForKey:@"appendInvert"] dataUsingEncoding:encoding]];
        else if ([command valueForKey:@"enableInvert"])
            [builder appendInvert:[[command valueForKey:@"enableInvert"] boolValue]];
        else if ([command valueForKey:@"appendUnderline"])
            [builder appendDataWithUnderLine:[[command valueForKey:@"appendUnderline"] dataUsingEncoding:encoding]];
        else if ([command valueForKey:@"enableUnderline"])
            [builder appendUnderLine:[[command valueForKey:@"enableUnderline"] boolValue]];
        else if ([command valueForKey:@"appendLineFeed"])
            [builder appendLineFeed:[[command valueForKey:@"appendLineFeed"] intValue]];
        else if ([command valueForKey:@"appendUnitFeed"])
            [builder appendUnitFeed:[[command valueForKey:@"appendUnitFeed"] intValue]];
        else if ([command valueForKey:@"appendLineSpace"])
            [builder appendLineSpace:[[command valueForKey:@"appendLineSpace"] intValue]];
        else if ([command valueForKey:@"appendFontStyle"])
            [builder appendFontStyle:[self getFont:[command valueForKey:@"appendFontStyle"]]];
        else if ([command valueForKey:@"appendCutPaper"])
            [builder appendCutPaper:[self getCutPaperAction:[command valueForKey:@"appendCutPaper"]]];
        else if ([command valueForKey:@"openCashDrawer"])
            [builder appendPeripheral:[self getPeripheralChannel:[command valueForKey:@"openCashDrawer"]]];
        else if ([command valueForKey:@"appendBlackMark"])
            [builder appendBlackMark:[self getBlackMarkType:[command valueForKey:@"appendBlackMark"]]];
        else if ([command valueForKey:@"appendBytes"]) {
            NSMutableArray *byteArray = [command valueForKey:@"appendBytes"];
            ...
            NSMutableArray *rawByteArray = [command valueForKey:@"appendRawBytes"];
            ...
            if ([command valueForKey:@"data"])
                [builder appendDataWithAbsolutePosition:[[command valueForKey:@"data"] dataUsingEncoding:encoding]
                                               position:[[command valueForKey:@"appendAbsolutePosition"] intValue]];
            else
                [builder appendAbsolutePosition:[[command valueForKey:@"appendAbsolutePosition"] intValue]];
        }
        else if ([command valueForKey:@"appendAlignment"]) {
            if ([command valueForKey:@"data"])
                [builder appendDataWithAlignment:[[command valueForKey:@"data"] dataUsingEncoding:encoding]
                                          position:[self getAlignment:[command valueForKey:@"appendAlignment"]]];
            else
                [builder appendAlignment:[self getAlignment:[command valueForKey:@"appendAlignment"]]];
        }
        else if ([command valueForKey:@"appendHorizontalTabPosition"]) {
            NSArray<NSNumber *> *tabPositionArray = [command valueForKey:@"appendHorizontalTabPosition"];
            if (tabPositionArray != nil && tabPositionArray != (id)[NSNull null])
                [builder appendHorizontalTabPosition:tabPositionArray];
        }
            ...
            if ([command valueForKey:@"logoSize"])
                [builder appendLogo:[self getLogoSize:[command valueForKey:@"logoSize"]]
                           number:[[command valueForKey:@"appendLogo"] intValue]];
            else
                [builder appendLogo:SCBLogoSizeNormal number:[[command valueForKey:@"appendLogo"] intValue]];
        }
            ...
            if ([command valueForKey:@"absolutePosition"]) {
                ...
                [builder appendBarcodeData:[[command valueForKey:@"appendBarcode"] dataUsingEncoding:encoding]
            ...
            if ([command valueForKey:@"absolutePosition"]) {
                ...
                [builder appendQrCodeData:[[command valueForKey:@"appendQrCode"] dataUsingEncoding:encoding]
...
- (SCBCodePageType)getCodePageType:(NSString *)codePageType {
    if (codePageType != nil && codePageType != (id)[NSNull null]) {
        ...
        else if ([codePageType isEqualToString:@"CP865"]) return SCBCodePageTypeCP1250; // Note: Adjust as needed
